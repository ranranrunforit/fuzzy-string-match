...
RFtrain <- BIRADStrainfeat[,c("pd","flatness", "skewness", "stdOverMeanIntensity",
...
RFtest <- BIRADStestfeat[,c("pd","flatness", "skewness", "stdOverMeanIntensity",
                             "compactness0","convexHull2Mask0","dense2convexHull0",         
                             "numObjects0","meanBlobSize0","stdOverMeanBlobSize0",
                             "stdOverDenseAreaBlobSize0","stdOverConvexHullBlobSize0",
                             "meanDenseTissue0","meanFatTissue0","stdDenseTissue0",          
...
#create RF Model
pd.rf <-randomForest(pd ~ ., data=RFtrain, ntree=1000,proximity=TRUE)
                        #keep.forest=TRUE, importance=TRUE,test=data$val)


# Run Prediction
pred <- predict(pd.rf,RFtest)
pred-RFtest[,1]
